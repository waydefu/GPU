...
權威現況：`HANDOFF.md`（裝置 experimental 為 `b984ded` CI **35347497216** INSTALLED。ADB **RESTORED** SERIAL `10.191.48.13:46847`。R8-C1 attempt-09 cell **NOT RUN**（目錄不存在）。歷史 preflight **R8_BLOCKED ADB_CONNECT_FAILED**（凍結，不准 retry `10.191.48.13:45165`）。R8 host tooling v2 **PUBLISHED**。R8-C1 attempt-08 **R8_INVALID JUDGE_NOT_PERMITTED / MULTI_BEGIN_x** frozen；attempts 01–07 frozen（含 attempt-05 **R8_INVALID** `POST_END_OBSERVATION`，X 14186）；C2–P2 NOT RUN。歷史 `fb4f017` CI **35338856846** 已在裝置上 superseded。歷史 `2a245b0` CI **35331185799** 已在裝置上 superseded。歷史 `5a782f6` CI **35321447455** 已在裝置上 superseded。歷史 `65938a4` CI **35311343984** 已在裝置上 superseded。歷史 `a4c8177` CI **35295094951** 已在裝置上 superseded。歷史 `abb27a65` CI **35253641841** 已在裝置上 superseded。歷史 `8545b26` CI **35225593518** 已在裝置上 superseded。歷史 `a07d66c` CI **35171333149** 已在裝置上 superseded。CASE_LOOP repair-validation **CASE_LOOP_REPAIR_VALIDATED**（非 B-2）。B-2 requalification-01 **INVALID** 凍結。B-2 requalification-02 **PASS**。歷史 R7 qualification-01 **BLOCKED**（r7-04 halt_mismatch，凍結）。R7-04 requalification-01 **PASS**（X 9891）。歷史 R7-05 **FAIL**（X 22704；last halt `r-hup reason=6`；凍結，不准 retry）。`a07d66c` R7-05 requalification-01 **PASS**（X 8418；last halt `x-direct-not-success/2`）。`a07d66c` R7-01 **PASS**（X 15029；last halt `r-gatea-DIRECT_LOOKUP_FAIL/2`）。`a07d66c` R7-02 **PASS**（X 28625；event35 enum=2；last halt `r-gatea-DIRECT_LOOKUP_FAIL/2`）。`a07d66c` R7-03 **PASS**（X 28326；event35 enum=3；last halt `r-gatea-direct-identity/5`）。`a07d66c` R7-06 **PASS**（X 21795；last halt `r-gatea-fence-create/3`）。`a07d66c` R7-07 **PASS**（X 31938；last halt `r-gatea-fence-wait/3`）。`a07d66c` R7-08 **PASS**（X 14424；last halt `r-test-fatal-pre-fence/6`）。`a07d66c` R7-09 **PASS**（X 22076；last halt `x-wrong-generation/6`）。R7-11 dedicated runner **QUALIFIED**；R7-11 device **PASS**（X 31764；last halt `x-serial-wrap/6`）。歷史 a07d66c R7-10 device **FAIL**（X 12568；last halt `x-direct-not-success/4`；RCA **PROVEN**；凍結）。repair `8545b26` R7-10 runner **QUALIFIED**；8545b26 R7-10 device **PASS**（X 13115；last halt `x-hup/6`）。歷史 R7-P1 **R7_P1_8545B26_QUALIFICATION_INVALID**（X 14331；classifier `NO_PRESENT_CALLBACK`；凍結）。abb27a65 R7-P1 **R7_P1_ABB27A65_QUALIFICATION_INVALID**（X 19686；classifier `PEER_DIED`；凍結，不准 retry）。a4c8177 R7-P1 **R7_P1_A4C8177_QUALIFICATION_INVALID**（X 24284；classifier `PEER_DIED`；凍結，不准 retry）。a4c8177 P1 validity-02 **R7_P1_A4C8177_QUALIFICATION_PASS**（X 1892）。a4c8177 P2 **R7_P2_A4C8177_QUALIFICATION_PASS**（X 12663）。R7 **COMPLETE 13/13**（do not retry runtime-a4c8177/r7-p1；do not retry runtime-a4c8177/r7-p1-validity-02；do not retry runtime-a4c8177/r7-p2；do not retry runtime-abb27a65/r7-p1；do not retry runtime-8545b26/r7-p1）。歷史 `feeaa56` stall-obs-01 **CASE_LOOP** 凍結。`327b028` SUPERSEDED。歷史 **R6 PASS** 在凍結 `0f1e546`。R8 design ACCEPTED；device `b984ded` INSTALLED CI **35347497216**；ADB **RESTORED** SERIAL `10.191.48.13:46847`；R8-C1 attempt-09 cell **NOT RUN**；historical preflight **R8_BLOCKED ADB_CONNECT_FAILED**；attempt-08 **R8_INVALID JUDGE_NOT_PERMITTED / MULTI_BEGIN_x** frozen；attempts 01–07 frozen（含 attempt-05 **R8_INVALID** `POST_END_OBSERVATION`，X 14186）；C2–P2 NOT RUN；historical CI **35304122983** / **35305368742 R8_CI_FAIL** frozen；do not silent-start C1；do not silent-retry CI **35304122983** / **35305368742** / **35311343984** / **35321447455** / **35331185799** / **35338856846** / **35347497216**；do not silent-retry `10.191.48.13:45165`；do not create attempt-10；do not retry `runtime-b984ded/r8-c1` or `runtime-fb4f017/r8-c1`；do not start R9。Production Gate A BLOCKED。）。
...
| Gate A P2 runtime（experimental `:3` only） | 禁裝／禁碰 | 裝置 `a4c8177` INSTALLED；repair-validation **CASE_LOOP_REPAIR_VALIDATED**（非 B-2）。B-2 requalification-02 **PASS**（X 32228）。`b2-requalification-01` **INVALID** 凍結。歷史 R7 qualification-01 **BLOCKED**（r7-04 X 31122 halt_mismatch）。R7-04 requal **PASS**（X 9891）。歷史 R7-05 **FAIL**（X 22704 last halt `r-hup/6`；凍結）。`a07d66c` R7-05 **PASS**（X 8418）。a07d66c one-cell R7 runner **QUALIFIED**。R7-01 **PASS**（X 15029）。R7-02 **PASS**（X 28625）。R7-03 **PASS**（X 28326）。R7-06 **PASS**（X 21795）。R7-07 **PASS**（X 31938）。R7-08 **PASS**（X 14424）。R7-09 **PASS**（X 22076）。R7-11 dedicated runner **QUALIFIED**；R7-11 device **PASS**。歷史 a07d66c R7-10 device **FAIL** frozen；RCA PROVEN；repair `8545b26` INSTALLED / R7-10 runner **QUALIFIED**；8545b26 R7-10 device **PASS**。歷史 R7-P1 **INVALID**。abb27a65 R7-P1 **INVALID**（X 19686 `PEER_DIED`）。a4c8177 first P1 **INVALID**（X 24284 `PEER_DIED`）。a4c8177 P1 validity-02 **PASS**（X 1892）。a4c8177 P2 **PASS**（X 12663）。R7 **COMPLETE 13/13**。歷史 `feeaa56` obs-01 **CASE_LOOP** 凍結。歷史 B-2 FAIL 在 `0d72332` 凍結。歷史 R6 PASS 在 `0f1e546`。 | **R6 PASS**（歷史）；CASE_LOOP repair **VALIDATED**；B-2 **PASS**；R7-04 **PASS**（`fdfb1ce`）；historical R7-05 **FAIL**；R7-05 **PASS**（`a07d66c`）；R7-01 **PASS**；R7-02 **PASS**；R7-03 **PASS**；R7-06 **PASS**；R7-07 **PASS**；R7-08 **PASS**；R7-09 **PASS**；R7-11 runner **QUALIFIED**；R7-11 device **PASS**；歷史 a07d66c R7-10 device **FAIL**；8545b26 R7-10 device **PASS**；歷史 P1 **INVALID**；a4c8177 P1/P2 **PASS**；R7 **COMPLETE 13/13**；Production Gate A **BLOCKED**；`b984ded` INSTALLED；ADB **RESTORED** `10.191.48.13:46847`；R8-C1 attempt-09 cell **NOT RUN**；attempt-08 **R8_INVALID** `MULTI_BEGIN_x` frozen（C2–P2 NOT RUN） |
