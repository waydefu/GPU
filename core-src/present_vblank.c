...
present_gpu_copy_retire_or_fatal(present_vblank_ptr vblank)
{
    ...
    if (!vblank)
        FatalError("Present GPU-copy retirement received NULL vblank");
    if (!vblank->gpu_copy_pending)
        return;

    serial = vblank->gpu_copy_serial;
    if (serial == 0 || !lorieGpuCopyIsDone(serial))
        lorieGpuCopyWaitForPresentOrFatal(serial);

    lorieGpuCopyAck(vblank->pixmap, vblank->gpu_copy_dst_buffer);
    vblank->gpu_copy_pending = FALSE;
    vblank->gpu_copy_dst_buffer = NULL;
    vblank->gpu_copy_serial = 0;
}
