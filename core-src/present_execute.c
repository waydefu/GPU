...
present_execute_copy(present_vblank_ptr vblank, uint64_t crtc_msc)
{
    ...
    if (vblank->gpu_copy_pending) {
        uint64_t gpuCopySerial = vblank->gpu_copy_serial;
        uint64_t gpuCopyDstId = lorieGateAPixmapBufferId(gpuCopyDst);

        if (!lorieGpuCopyIsDone(gpuCopySerial)) {
            if (!gpuCopyStalled &&
                Success == screen_priv->queue_vblank(screen, window, vblank->crtc,
                                                     vblank->event_id, crtc_msc + 1)) {
                vblank->queued = TRUE;
                return;
            }
            if (!gpuCopyStalled) {
                /* Natural queue_vblank failure after a prior schedule follows
                 * the same completion-before-ACK contract as the R6 one-shot. */
                lorieGateATracePresentRequeueFailed(gpuCopySerial, gpuCopyDstId);
                present_gpu_copy_retire_or_fatal(vblank);
                lorieGateATracePresentAckAfterCompleted(gpuCopySerial, gpuCopyDstId);
            } else {
                /* The helper fail-stops if completion is still uncertain. */
                present_gpu_copy_retire_or_fatal(vblank);
            }
        } else {
            present_gpu_copy_retire_or_fatal(vblank);
        ...
        uint64_t gpuCopyDstId;
        ClientPtr gateAClient;
        uint32_t gateAClientSeq;
        vblank->gpu_copy_pending = TRUE;
        gpuCopySerial = vblank->gpu_copy_serial;
        gpuCopyDstId = lorieGateAPixmapBufferId(gpuCopyDst);
        gateAClient = GetCurrentClient();
        gateAClientSeq = gateAClient ? gateAClient->sequence : 0;
        ...
        lorieGateATraceXCallback(4u, gpuCopySerial, gateAClientSeq);
        if (vblank->update)
        ...
        if (!lorieGateAPresentRequeueShouldFail() &&
            Success == screen_priv->queue_vblank(screen, window, vblank->crtc, vblank->event_id, crtc_msc + 1)) {
            vblank->queued = TRUE;
            return;
        }
        /* Post-schedule requeue failed (OOM or R6 one-shot). Keep pending
         * refs until GPU completion is proven, then ACK. Timeout or renderer
         * loss fail-stops without releasing ownership. */
        lorieGateATracePresentRequeueFailed(gpuCopySerial, gpuCopyDstId);
        present_gpu_copy_retire_or_fatal(vblank);
        lorieGateATracePresentAckAfterCompleted(gpuCopySerial, gpuCopyDstId);
    } else {
