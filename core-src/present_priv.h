...
extern void lorieGpuCopyWaitForPresentOrFatal(uint64_t serial);
void present_gpu_copy_retire_or_fatal(present_vblank_ptr vblank);
extern uint64_t lorieGateAPixmapBufferId(PixmapPtr pixmap);
extern void lorieGateATraceXCallback(uint32_t xop, uint64_t gpuSerial, uint32_t clientSeq);
extern int lorieGateAPresentRequeueShouldFail(void);
extern void lorieGateATracePresentEarlyAck(uint64_t gpuSerial, uint64_t dstId);
extern void lorieGateATracePresentRequeueFailed(uint64_t gpuSerial, uint64_t dstId);
extern void lorieGateATracePresentAckAfterCompleted(uint64_t gpuSerial, uint64_t dstId);
