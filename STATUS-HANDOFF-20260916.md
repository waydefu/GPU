...
CASE_LOOP fix: 327b028 initial → hardened 7549e36 CI 35084701124 QUALIFIED / **未安裝**
...
不准安裝 `7549e36`（需新授權）。不准 retry `runtime-feeaa56/stall-obs-01`。不准開 B-2 矩陣。不准改 timeout。不准開 R7 qualification。
...
- CASE_LOOP initial `327b028` then hardened `7549e36`（CLOCK_MONOTONIC + C11 atomics；host RED 151 ms / GREEN 10 ms）。

---

## 還沒做

- 安裝 `7549e36`（需新授權：artifact QUALIFIED 後 experimental only）。
- B-2 重資格（仍 BLOCKED）。
