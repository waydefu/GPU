...
run_variant() {   # name port use_mesa(0/1) flags...
  local name=$1 port=$2 mesa=$3; shift 3
  local dir=$PROF/$name; rm -rf "$dir"; mkdir -p "$dir"
  local envs=(); [ "$mesa" = 1 ] && envs=("${MESA_ENV[@]}")
...
run_variant v0-control 9230 0 --disable-gpu
run_variant v1-angle-gl 9231 1 --ignore-gpu-blocklist --use-gl=angle --use-angle=gl-egl
run_variant v2-angle-vk 9232 1 --ignore-gpu-blocklist --use-gl=angle --use-angle=vulkan \
