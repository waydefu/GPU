#!/usr/bin/env bash
# RCA pair for the xfce2-c1-02 failure (NOT baseline runs, never pooled with them).
...
for arm in ${ARMS:-G C}; do
  ev=$HERE/runtime-bfb5769/rca-${arm,,}-01
  ...
  VARIANT=C1 SERIAL=$SERIAL EVIDENCE=$ev "$HERE/run-xfce-v2.sh" > "$ev.runner.log" 2>&1
  rc=$?
  wait $SP
