#!/usr/bin/env bash
# XFCE baseline runner — product dc94485, XFCE-FREEZE-V1. ORCHESTRATES AND CAPTURES ONLY.
#
...
XFCE=$ROOT/src/f8-ahb-gatea-r7-p1-arm/tests/xfce
...
EXPECT_MANIFEST_SHA=a3eeaf9de97e93802ce1bdb1a8e96b58559a7354d3cdd5061f70e29e75bbaadb
...
if [ "$VALIDATE_ONLY" = 1 ]; then
  echo "XFCE_RUNNER_V1_VALIDATE_ONLY variant=$VARIANT manifest=$GOT"
  exit 0
fi
...
LAUNCHER_OFF=$(stat -c %s "$X3_LAUNCHER_LOG" 2>/dev/null || echo 0)
echo "launcher_log_offset=$LAUNCHER_OFF" > "$EVIDENCE/launcher-offset.txt"
