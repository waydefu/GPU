#!/usr/bin/env bash
# V2-B3 runner — product bfb5769, B3_FROZEN_V1. CAPTURES ONLY (analysis: tests/b3/b3_analyze.py).
#
#   MODE      G | C | S | G2 (timing, production config) | ATTR (G attribution, telemetry on)
#   EVIDENCE  must not exist · SERIAL live 5038-lane serial · VALIDATE_ONLY=1 = no device use
#
# Timing modes run WITHOUT telemetry and WITHOUT the R8 arm (production config: a per-event
# liblog write would distort small-rect timings). There is then no test-control terminate,
# so X is ended with SIGTERM by exact pid AFTER the fixture has exited - CONSTRUCTION,
# recorded in x-end.txt; B.3 judges performance and pixels, not lifecycle.
# ATTR runs G with TELEMETRY=1 + R8-C1 arm, 3 iterations per cell, clean terminate, so
# every cell's window can be matched to its GATEA_EVENT event=5 count.
...
# be583454... -> ea506447...: + b3_attribution (streaming) and its tests, safe-run.sh; same freeze
# ea506447... -> 9b66082b...: + mem-guard.sh (INCIDENT-20260923-2), same freeze
# 9b66082b... -> 034683e8...: fixture --quiet-ms (ATTR only) + b3_attribution V2; timing modes unchanged
EXPECT_B3_MANIFEST_SHA=034683e89b30d95fddbeffcf384f395697cd0b971175b15065fc341c083d7957
EXPECT_ROOT=1200x2191

refuse() { echo "B3_BLOCKED $*"; [ -n "${EVIDENCE:-}" ] && [ -d "$EVIDENCE" ] && echo "$*" > "$EVIDENCE/BLOCKED.txt"; exit 3; }
invalid() { echo "B3_INVALID $*"; echo "$*" > "$EVIDENCE/INVALID-CAPTURE.txt"; exit 2; }
MODE="${MODE:-}"; EVIDENCE="${EVIDENCE:-}"
case "$MODE" in G|C|S|G2|ATTR) ;; *) refuse "bad_mode '$MODE'";; esac
[ -n "$EVIDENCE" ] || refuse "EVIDENCE"
[ "$(sha256sum "$B3/FROZEN-SHA256SUMS" | awk '{print $1}')" = "$EXPECT_B3_MANIFEST_SHA" ] || refuse "manifest_sha"
( cd "$B3" && sha256sum --quiet -c FROZEN-SHA256SUMS ) || refuse "frozen_file_changed"
if [ "${VALIDATE_ONLY:-0}" = 1 ]; then echo "B3_RUNNER_V1_VALIDATE_ONLY mode=$MODE"; exit 0; fi
...
mkdir -p "$EVIDENCE"
sha256sum "$0" "$B3/FROZEN-SHA256SUMS" "$FIXTURE" "$TERMINATE" > "$EVIDENCE/tools.sha256.txt"
X3=""; XEND=""
b_cleanup() {
...
LLOG=/data/data/com.termux/files/usr/tmp/x3-launch-b3-$MODE-$$.log
...
set +e
DISPLAY=:3 timeout 3600 "$FIXTURE" --cells "$B3/cells.tsv" --seed "$SEED" --iters "$ITERS" \
...
if [ -e "$EVIDENCE/MEM-GUARD-TRIPPED.txt" ]; then
  (cd "$EVIDENCE" && find . -type f ! -name sha256sums.txt | sort | xargs sha256sum > sha256sums.txt)
  echo "B3_ABORTED_MEM_GUARD evidence=$EVIDENCE $(head -1 "$EVIDENCE/MEM-GUARD-TRIPPED.txt")"
  exit 5
fi
echo "B3_CAPTURED mode=$MODE evidence=$EVIDENCE cells=$(grep -c '^CELL ' "$EVIDENCE/b3-cells.out")"
