...
EXPECT_B3_MANIFEST_SHA=be583454ac7aedde11d40d5012ad32343546557447e7020f56a693760bfd8fc2
...
DISPLAY=:3 timeout 3600 "$FIXTURE" --cells "$B3/cells.tsv" --seed "$SEED" --iters "$ITERS" \
  --warmup "$WARMUP" --noise-every 25 --x-pid "$X3" > "$EVIDENCE/b3-cells.out" 2>&1
