...
env MESA_LOADER_DRIVER_OVERRIDE=zink GALLIUM_DRIVER=zink VK_ICD_FILENAMES=$TU_ICD $TB/glxinfo -B > "$OUT/glxinfo-zink.txt" 2>&1
env VK_ICD_FILENAMES=$TU_ICD $TB/glxinfo -B > "$OUT/glxinfo-default.txt" 2>&1
...
run gears-zink 12 env MESA_LOADER_DRIVER_OVERRIDE=zink GALLIUM_DRIVER=zink VK_ICD_FILENAMES=$TU_ICD $TB/glxgears
for f in llvmpipe zink; do echo "gears-$f: $(grep -E 'frames in' "$OUT/gears-$f.log" | tail -2 | tr '\n' ' ')"; done
echo "== vkcube 600 frames"
run vkcube-turnip 60 env VK_ICD_FILENAMES=$TU_ICD $TB/vkcube --c 600
run vkcube-lavapipe 120 env VK_ICD_FILENAMES=$LVP_ICD $TB/vkcube --c 600
