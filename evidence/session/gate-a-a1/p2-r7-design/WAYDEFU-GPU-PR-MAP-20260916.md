...
main 2ab76b30  ← merge of PR #3 (contains unique R6 files from #1)
  ...
  └─ PR #4  case-loop-feeaa56-20260916      OPEN (this snapshot)
```
...
| [#4](https://github.com/waydefu/GPU/pull/4) | `case-loop-feeaa56-20260916` | `feeaa56` **CASE_LOOP** + source `327b028` **not installed** | **this PR** |

## Verdict labels (do not mix)

- **R6 = PASS** (frozen `0f1e546`)
- **B-2 = BLOCKED** (`0d72332` fail-stop; historical `a7528bd` FAIL kept)
- **feeaa56 stall-obs-01 = CASE_LOOP** — **not** a B-2 PASS
- **CASE_LOOP fix `327b028` = COMMITTED, not installed, not CI**
- **R7 support artifact** `a7528bd` exists; **R7 qualification NOT STARTED**

Docs merge is **not** R7 qualification, **not** a B-2 PASS, and **not** device install of `327b028`.
