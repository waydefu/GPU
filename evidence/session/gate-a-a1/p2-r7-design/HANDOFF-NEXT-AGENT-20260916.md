...
STATUS: R6 PASS on frozen 0f1e546 remains historical
        ...
        stall RCA: CASE_LOOP DEVICE-PROVEN on feeaa56 stall-obs-01
...
CASE_LOOP WT: src/f8-ahb-gatea-case-loop HEAD 7549e36 branch fix/gatea-case-loop-wakeup-20260916
CASE_LOOP CI: 35084701124 QUALIFIED / NOT INSTALLED
...
4. DEVICE-PROVEN: CASE_LOOP consume stall (published S not consumed for ~2 s).
   SOURCE-SUPPORTED: `waitForNextFrame` + lost wakeup / unbounded idle wait.
   Direct `pthread_cond_wait` instruction for the whole 2 s was **not**
   instrumented (`COND_WAIT_*` absent).
5. Hardened source `7549e36`: `waitWhileIdle` rechecks writeIndex and uses
