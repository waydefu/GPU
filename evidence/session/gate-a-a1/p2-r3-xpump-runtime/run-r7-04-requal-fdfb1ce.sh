#!/usr/bin/env bash
# One R7-04 requalification cell on installed fdfb1ce. Experimental :3 only.
# Not B-2. Not historical r7-qualification-01. Not R7-05+. No source mutation. No retry.
# Usage: SERIAL=... ROOT=.../runtime-fdfb1ce \
#        CELL_ID=r7-04-requalification-01 FAULT=fbo-incomplete FIXTURE=direct WAIT_S=12 \
#        bash run-r7-04-requal-fdfb1ce.sh
set -euo pipefail
...
case "$CELL" in
  ...
  *) echo "REFUSE CELL must be .../runtime-fdfb1ce/r7-04-requalification-01 got=$CELL"; exit 9;;
esac
...
import re, sys
cell = Path(sys.argv[1])
follow = (cell/'logcat-follow.txt').read_text(errors='replace')
dump = (cell/'logcat-dump-unfiltered.txt').read_text(errors='replace') if (cell/'logcat-dump-unfiltered.txt').exists() else ""
src = follow if "GATEA_FATAL_HALT" in follow else dump
halts = re.findall(r"GATEA_FATAL_HALT what=(\S+) reason=(\d+)", src)
observe = len(re.findall(r"x-observe-fatal", follow + "\n" + dump))
xdns4 = [h for h in halts if h[0] == "x-direct-not-success" and h[1] == "4"]
xdns_any = [h for h in halts if h[0] == "x-direct-not-success"]
lines = [
    f"halt_count={len(halts)}",
    f"halts_chrono={halts}",
    f"final_halt={halts[-1] if halts else None}",
    f"x_direct_not_success_reason4={len(xdns4)}",
    f"x_direct_not_success_any={len(xdns_any)}",
    f"x_observe_fatal_dump={observe}",
]
(cell/'halt-propagation-audit.txt').write_text("\n".join(lines) + "\n")
print("halt_propagation", "\n".join(lines))
PY2
