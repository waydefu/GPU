#!/usr/bin/env bash
# Install PGA-GAP-1 fix APK bfb5769 (CI 35811368916) onto experimental
# com.waydefu.x11gpu only. DOES NOT RUN unless RUN_INSTALL=YES.
# Never touches com.termux.x11 / :1 / HDMI.
# Usage: RUN_INSTALL=YES SERIAL=HOST:PORT CELL=.../artifact-bfb5769/install bash install-bfb5769.sh
set -euo pipefail

ROOT=/root/projects/GPU加速/evidence/session/gate-a-a1/p2-r3-terminal-runtime
APK=/root/projects/GPU加速/evidence/session/gate-a-a1/p2-pga-artifact/artifact-bfb5769/ci-35811368916/termux-x11-universal-debug.apk
EXPECT_SHA256=ff7b9309c405b07245bc352911867a66e8498464e515e09dbd645b3a1ec18472
EXPECT_BUILDID=890db760a77a7d22945b9640705f70d025b5d518
EXPECT_VERSION=1.03.01-bfb5769-23.09.26
EXPECT_SIZE=15381006
...
EXPECT_PARENT=dc94485a7ef4f74cada36ea3c1d35d0aa0f48693
EXPECT_CI=35811368916
REMOTE=/data/local/tmp/gate-a-pga-bfb5769-35811368916.apk
APK_FOR_SIGNER=/tmp/pga-bfb5769-termux-x11-universal-debug.apk
...
case "$CELL" in
  */artifact-bfb5769/install) ;;
  *) echo "REFUSE CELL must be .../artifact-bfb5769/install got=$CELL"; exit 9;;
esac
mkdir -p "$CELL"
...

{
  ...
  echo "role=bfb5769-pga-gap1-install"
...
cp -a "$CELL/installed-base.apk" /tmp/pga-bfb5769-installed-base.apk
/usr/bin/apksigner verify --verbose --print-certs /tmp/pga-bfb5769-installed-base.apk \
...
test "$(stabpid)" = "$STABLE_PRE"
echo INSTALL_BFB5769_BIND_PASS | tee "$CELL/install-bind-pass.txt"
