#!/usr/bin/env bash
# XFCE PRE-FREEZE ENVIRONMENT PROBE — product dc94485. NOT an XFCE session.
#
...
while IFS= read -r L; do
  name=$(echo "$L" | sed -E 's/^RequestedLayerState\{//; s/ parentId=.*$//; s/\}$//')
  [ -n "$name" ] || continue
  { echo "=== layer: $name"; ADB shell dumpsys SurfaceFlinger --latency "\"$name\"" 2>&1 | head -140; } \
    >> "$EVIDENCE/sf-latency.txt"
