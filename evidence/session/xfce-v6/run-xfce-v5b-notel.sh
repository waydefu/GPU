#!/usr/bin/env bash
# XFCE runner V5b for XFCE-NOTEL-01 deviation 1 (freeze section 10): V5 with R8 NOT armed. The product halts at start
# (x-r8-env, lorie_r8_obs.c:130 lorieR8ValidateStartupEnv) when R8 is armed without telemetry, so no group arms R8; the
# LORIE-R8-TEST extension is then absent, the clean terminate fails and the EXIT trap's xfce_cleanup closes X3 (SIGTERM to
# the verified pid + force-stop, recorded in cleanup.txt) after the measurement.
# --- V5 header follows ---
# XFCE runner V5 for XFCE-NOTEL-01 (evidence/session/xfce-v6/XFCE-NOTEL-01-FREEZE.md), derived from V4 with exactly two
# changes: (a) the client tracer must be proot-fast7 (sha256 4f9d10ad..., the daily tracer since 2026-09-25) instead of
# proot-fast6; (b) TERMUX_X11_GATEA_TELEMETRY comes from TELEMETRY=0|1 (required) instead of being hard-coded to 1.
#   TELEMETRY  0 = unset TERMUX_X11_GATEA_TELEMETRY (groups C, GT), 1 = export it =1 (group CT)
# --- V4 header follows ---
...
V6_TRACER_SHA=4f9d10ad2f78e1eef3907eea1ef761b4b8265c9d7cbd5779df48b4f8cb756058   # V5: proot-fast7 (variable name kept from V4)
...
case "${TELEMETRY:-}" in 0|1) ;; *) refuse "TELEMETRY must be 0 or 1";; esac   # V5
...
if [ "$VALIDATE_ONLY" = 1 ]; then
  echo "XFCE_RUNNER_V5B_VALIDATE_ONLY mode=$MODE variant=$VARIANT manifest=$GOT"
  exit 0
fi
...
if [ "$TELEMETRY" = 1 ]; then export TERMUX_X11_GATEA_TELEMETRY=1; else unset TERMUX_X11_GATEA_TELEMETRY; fi   # V5
unset TERMUX_X11_R8_ARM TERMUX_X11_R8_CASE   # V5b: R8 armed without telemetry halts X (x-r8-env)
