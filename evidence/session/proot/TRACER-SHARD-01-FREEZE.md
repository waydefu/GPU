...
## 12. 結果（2026-09-26；本節在資料之後寫，§1–§11 不改）

### 12.1 執行

- 日常追蹤器已換 proot-fast7：2026-09-25 21:37 使用者自己執行 `apply-v7-f8desk.sh`（f8desk／f8desk-external 第一順位 out7，備份 `*.bak-20260925-v7`），
  再用「F8 工作站」重開桌面。開跑前核對：client 追蹤器 pid 29525 exe sha256 `4f9d10ad…`、無關閉開關；5 個工具 sha256 與 §11 相同；
  跨追蹤器 D-Bus 對**真實日常 session bus** 實測成功（分流追蹤器下 `GetId` 得到與日常相同的 bus id `df8deb88…`）。
- 一條指令：`SERIAL=10.191.48.13:44925 OUT=evidence/session/proot/tracer-shard-01 bash .../series-shard.sh`，2026-09-26 00:49:21 開始、01:30:52 判定完成。
  開跑前：AC 充電、`mStayOn=true`、螢幕亮、未鎖、MemAvailable 4607 MB；只有 Claude 在跑。
  （開跑前 ADB 兩次失效：手機網路 192.168.1.100 → 10.191.48.13，無線偵錯被關；使用者重開後以埠掃描找到新端點。）

| 格 | 組 | loaded 窗 | 有效 | S1 | S2 | S3 | E |
|---|---|---|---|---|---|---|---|
| shard-p-01 | P | 00:51:49–00:55:49 | **INVALID**（`touch events 755`，使用者當下回報不小心碰到手機） | — | — | — | — |
| shard-s-01 | S | 00:57:34–01:01:35 | 有效 | 0 | 0 | 0 | **0** |
| shard-s-02 | S | 01:03:21–01:07:21 | 有效 | 0 | 0 | 0 | **0** |
| shard-p-02 | P | 01:09:08–01:13:09 | 有效 | 20 | 0 | 0 | 20 |
| shard-p-03 | P | 01:14:54–01:18:55 | 有效 | 20 | 0 | 0 | 20 |
| shard-s-03 | S | 01:20:41–01:24:41 | 有效 | 0 | 0 | 0 | **0** |
| shard-p-r1 | P | 01:26:28–01:30:29 | 有效（p-01 的補跑，§5；用掉 2 次中的 1 次） | 18 | 0 | 0 | 18 |

INVALID 的 p-01 無資訊，不列入判準與描述；目錄原樣保留。

### 12.2 判定（`tracer-shard-01/series-judge.json`）

- **T0 `CONTROL_REPRODUCES`**：P 的 E = 20、20、18，最小 18 ≥ 3。
- **T1 `SHARD_REMOVES_STUTTER`**：3 次 S 的 E 都 = 0。
- **T2 `X_SERVER_NO_STALLS`**：3 次 S 的 S3 都 = 0（P 也都 0）。
- verdict **`JUDGED`**。依 §8：提議把日常 `cursor-gpu`、`hermes-gpu` 改經 `f8-shard`，由使用者決定，不自動改。

### 12.3 描述（不改判決）

| 格 | 日常追蹤器核數 平均／峰值（loaded） | 分流追蹤器 Cursor／Hermes 平均（峰值） | fstatat p99（µs） | getppid p99（µs） | 被追蹤 rtt p99（ms） |
|---|---|---|---|---|---|
| p-02 | 0.256／1.080 | — | 71387 | 6.7 | 2.16 |
| p-03 | 0.248／1.161 | — | 79946 | 4.2 | 2.02 |
| p-r1 | 0.232／0.943 | — | 77937 | 4.4 | 4.70 |
| s-01 | **0.041**／0.397 | 0.134（0.945）／0.029（0.455） | **1117** | 4.8 | 2.49 |
| s-02 | **0.042**／0.437 | 0.165（0.954）／0.030（0.497） | **989** | 4.4 | 2.36 |
| s-03 | **0.019**／0.445 | 0.170（0.933）／0.035（0.517） | **1375** | 5.2 | 2.14 |

- 機制與假說一致：P 組日常追蹤器在啟動時到 0.94–1.16 核（單執行緒滿載），留在它底下的 fstatat p99 71–80 ms；
  S 組 Cursor 的啟動負載整個到它自己的追蹤器（峰值 0.93–0.95 核，在那裡滿載），日常追蹤器平均 0.02–0.04 核，fstatat p99 約 1 ms（約 1/60）。
- 本輪 P 的 E 全部是 S1（S2 = 0）：v7 日常下，被追蹤探測的 X 往返沒有 > 100 ms。
- loaded 窗結束時 renderer 數：P 各 2、S 各 1（`apps-end.json`）。§4 只要求 ≥ 1；app 自己在分流追蹤器上是否較慢（啟動到可用的時間）本輪未量，只記錄。
- 關閉：P 經 `--close-launched`（Cursor 1.5 s、Hermes 7.5 s 內結束），S 經 f8-shard 轉送 SIGTERM（Cursor 1.5–2.0 s、Hermes 8.0 s），
  每格結束只剩 Claude；分流追蹤器全部結束（系列後只剩日常追蹤器 29525 與它的 `proot --shm-helper` 29680）。
- loaded 窗 MemAvailable 最低 3635–3934 MB、swap 最高 7554–7959 MB（3 個 Electron）。
- 與 XFCE-V6-BASELINE-01 Part A（v6，E = 48、14）追蹤器版本與工具不同，不合併，只記方向。
