...
one v5off $B/out5/bin/proot-fast5 PROOT_STAT_AT_ENTER=0
for l in v5 v5off; do diff $B/v5smoke-v2.txt $B/v5smoke-$l.txt > $B/v5smoke-diff-$l.txt && echo "$l IDENTICAL to v2 ($(wc -l < $B/v5smoke-v2.txt) lines)" || { echo "$l DIFFERS"; head -20 $B/v5smoke-diff-$l.txt; }; done
